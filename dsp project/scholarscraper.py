...
threshold = 40
path_of_xlsx = os.getcwd() + "\\DSv4.1.xlsx"
...
def scrape_is_go(df, start, stop):
    ...
    with open("file{}-{}.csv".format(start, stop), "w+", encoding='utf-8') as file:
        ...
        for c, name in enumerate(names, 1):
            ...
            print("\n{}/{} records done".format(c,len(names)))
            sleep(3)
    return
...
def scrape_individual(name, max_publications_per_author):
    ...
    try:
        ...
        print("\n{} got no records. Try manually.".format(name))
        return (name, "", "", "", "", "", "", "", "", "", "", "")
    author = query.fill()
    try:
    ...
    print("{} got {} publications".format(name, len(total_publications)))
